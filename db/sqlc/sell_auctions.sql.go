...
SELECT id, public_id, owner_id, region_id, interest_id, title, description, image_url, unit, quantity, unit_price, buy_all_from_one, bid_count, end_time, status, selected_bid_id, winner_id, final_price, created_at, updated_at, owner_name, region_name, interest_name, notified_at, last_motivation_sent_at FROM sell_auctions
WHERE status = 'pending_selection' AND end_time + INTERVAL '1 hour' <= NOW()
`

func (q *Queries) GetExpiredPendingSelectionSellAuctions(ctx context.Context) ([]SellAuction, error) {
	rows, err := q.db.Query(ctx, getExpiredPendingSelectionSellAuctions)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) GetSoonExpiringSelectionSellAuctions(ctx context.Context) ([]SellAuction, error) {
	rows, err := q.db.Query(ctx, getSoonExpiringSelectionSellAuctions)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
