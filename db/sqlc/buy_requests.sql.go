...
func (q *Queries) GetExpiredPendingSelectionBuyRequests(ctx context.Context, windowHours int32) ([]BuyRequest, error) {
	rows, err := q.db.Query(ctx, getExpiredPendingSelectionBuyRequests, windowHours)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) GetSoonExpiringSelectionBuyRequests(ctx context.Context, windowHours int32) ([]BuyRequest, error) {
	rows, err := q.db.Query(ctx, getSoonExpiringSelectionBuyRequests, windowHours)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const markBuyRequestSelectionWarned = `-- name: MarkBuyRequestSelectionWarned :exec
UPDATE buy_requests SET selection_warning_sent_at = NOW() WHERE id = $1
`

func (q *Queries) MarkBuyRequestSelectionWarned(ctx context.Context, id int32) error {
	_, err := q.db.Exec(ctx, markBuyRequestSelectionWarned, id)
	return err
}
